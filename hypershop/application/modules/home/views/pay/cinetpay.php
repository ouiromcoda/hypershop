...
class cinetpay
{
    ...
    const URI_WEBSITE_DEV = 'www.cinetpay.com';
    const URI_CASH_DESK_PROD = 'secure.cinetpay.com';
    const URI_CASH_DESK_DEV = 'secure.d.cinetpay.com/';
    /**
     * An identifier
     * @var string
     */
    ...
    public function __construct($site_id, $apikey, $mode = "PROD", $version = 'v2', $params = null)
    {
        ...
        if ($mode == "PROD") {
            ...
            $version = 'dev';
            $this->_URI_GET_SIGNATURE_DEV = sprintf('webservice.cinetpay.com/%s/?method=getSignatureByPost', strtolower($version));
            $this->_URI_CHECK_PAY_STATUS_DEV = sprintf('webservice.cinetpay.com/%s/?method=checkPayStatus', strtolower($version));
            $this->_cfg_cpm_version = 'V1';
        }
    ...
    public function getOnlyPayButtonToSubmit($formName, $btnType = 1, $size)
    {
        ...
        elseif (!empty($formName) && $btnType == 2)
            $btn = "<button class='btn btn-primary checkout-btn' onclick='javascript:document.forms['" . $formName . "'].submit()'> Procéder au paiement </button>";
        ...
        elseif (!empty($formName) && $btnType == 4)
            ...
            $btn = "<button class='btn btn-primary checkout-btn' onclick='javascript:document.forms['" . $formName . "'].submit()'> Payer maintenant </button>";
        return $btn;
    }
